...
The workflow for each fix or feature: write the change on a `fix/*` or `feat/*` branch, validate with the native test suite, build firmware, flash to the car (RHR893), verify on the vehicle, then open a PR upstream. Keep each branch/PR to one thing.
